...
type Rules = HashMap<String, HashSet<i32>>;
...
impl Ticket {
    ...
    fn error_rate(&self, rules: &Rules) -> i32 {
        self.fields
            .iter()
            .filter(|v| Ticket::find_candiates(**v, rules).is_empty())
            .sum()
    }
    ...
    fn is_valid(&self, rules: &Rules) -> bool {
        !self.fields
            .iter()
            .any(|v| Ticket::find_candiates(*v, rules).is_empty())
    }

    fn find_candiates(v: i32, rules: &Rules) -> HashSet<String> {
...
enum Section { Rules, Ticket, Nearby }
        ...
        match section {
            Section::Rules => {
                ...
                for cap in re.captures_iter(line) {
                    ...
                    let stop: i32 = cap[2].parse().unwrap();

                    let allowed = data.rules.entry(field.to_string()).or_insert(HashSet::new());
                    for x in start..=stop {
                        allowed.insert(x);
                    }
                }
            ...
            Section::Ticket => {
                let vals = line.split(",").map(|s| s.parse().unwrap()).collect();
                data.ticket = Ticket { fields: vals };
            },
            Section::Nearby => {
                let vals = line.split(",").map(|s| s.parse().unwrap()).collect();
                data.nearby.push(Ticket { fields: vals });
            }
        }
    }
